...
const getBooks = (callback) => {
    pool.getConnection((err, connection) => {
        if (err) {
            ...
            return callback({ error: 'Connect Database error', details: err.stack }, null);
        }

        connection.query('SELECT * FROM myappdb.books', (error, results) => {
            connection.release();
            if (error) {
                console.error('Error querying database: ', error.stack);
                return callback({ error: 'Query Database error', details: error.stack }, null);
            }
...
app.get('/api/books/:id', (req, res) => {
    ...
    pool.getConnection((err, connection) => {
        if (err) {
            console.error('Error connecting to database: ' + err.stack);
            return res.status(500).json({ error: 'Connect Database error' });
        }
        ...
        connection.query('SELECT * FROM books WHERE id = ?', [bookId], (error, results, fields) => {
            ...
            if (error) {
                console.error('Error querying database: ' + error.stack);
                return res.status(500).json({ error: 'Query Database error' });
            }
